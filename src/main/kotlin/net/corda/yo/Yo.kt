...
class YoApi(val rpcOps: CordaRPCOps) {
    ...
    fun yo(@QueryParam(value = "target") target: List<String>): Response {
        val (status, message) = try {
            ...
            for(i in target.indices) {
                ...
                val matches = rpcOps.partiesFromName(target[i], exactMatch = true)
            ...
            val result = rpcOps.startFlowDynamic(YoFlow::class.java, acceptors).returnValue.getOrThrow()
            // Return the response.
            Response.Status.CREATED to "You just sent a Yo!} (Transaction ID: ${result.tx.id})"
        } catch (e: Exception) {
...
@InitiatingFlow
@StartableByRPC
class YoFlow(val target: List<Party>) : FlowLogic<SignedTransaction>() {
...
data class YoState(val origin: Party,
                 val target: List<Party>,
                 val yo: String = "Yo!") : ContractState, QueryableState {
    override val participants: List<AbstractParty> get() = target
    ...
    override fun generateMappedObject(schema: MappedSchema) = YoSchemaV1.PersistentYoState(
            origin.name.toString(), target.toString(), yo)
